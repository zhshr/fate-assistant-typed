...
```
git clone https://github.com/zhshr/fate-assistant-typed.git
npm install
npm start
```
This will start development server at [`localhost:3000`](http://localhost:3000). If need to change default port, use `PORT=3001 npm start`

## Code structure
- `src/`
  - Contains boilerplate code generated by `create-react-app`. Specifically `App.tsx`
- `src/FA/`
  - This folder contains main code logic. FA stands for Fate Assistant
- `src/FA/Eirei`
  - Contains UI(EireiCard) and data(EireiData)
- `src/FA/Map`
  - Main UI and logic for campus UI. Including the click-and-move logic.
- `src/FA/SynchronizeUtils`
  - Responsible for state synchronizion across master/slave apps, i.e. map status and ping. 
